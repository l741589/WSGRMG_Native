...
static int do_load(int fd, symtab_t symtab)
{
	...
	for (i = 0, p = shdr; i < ehdr.e_shnum; i++, p++)
		if (SHT_SYMTAB == p->sh_type) {
			if (symh) {
				log("too many symbol tables 1\n");
				LOGR("too many symbol tables 1\n");
				goto out;
			}
			symh = p;
		} else if (SHT_DYNSYM == p->sh_type) {
			if (dynsymh) {
				log("too many symbol tables 2\n");
				LOGR("too many symbol tables 2\n");
				goto out;
			}
			dynsymh = p;
			...
			if (strh) {
				log("too many string tables 3\n");
				LOGR("too many string tables 3\n");
				goto out;
			}
			strh = p;
		} else if (SHT_STRTAB == p->sh_type
	...
	if ((!dynsymh && dynstrh) || (dynsymh && !dynstrh)) {
		...
		LOGR("bad dynamic symbol table\n");
		goto out;
	}
	if ((!symh && strh) || (symh && !strh)) {
		log("bad symbol table\n");
		LOGR("bad symbol table\n");
		goto out;
	}
	if (!dynsymh && !symh) {
		log("no symbol table\n");
		LOGR("no symbol table\n");
		goto out;
	}
