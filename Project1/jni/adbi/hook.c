...
int hook_a(struct hook_t *h, unsigned long int addr, char *funcname, void *hook_arm, void *hook_thumb){
	int i;
	...
	if (addr % 4 == 0) {
		...
		if ((unsigned long int)hook_thumb % 4 == 0) {
			...
			LOGR("warning hook is not thumb 0x%lx\n", (unsigned long)hook_thumb);
		}
		h->thumb = 1;
...
int hook(struct hook_t *h, int pid, char *libname, char *funcname, void *hook_arm, void *hook_thumb)
{
	...
	return hook_a(h,addr,funcname,hook_arm,hook_thumb);	
}
